...
${CONDA} run -n op --no-capture-output \
    ...
    --training-file data/moral_stories/critic_train_prep_final.json \
    --validation-file data/moral_stories/critic_dev_prep_final.json \
    --language-model allenai/unifiedqa-t5-base \
    --model-dir /scratch/mete/op/op_critic_moral_stories_reborn \
    --epochs 5 \
    --batch-size 32 \
    --val-batch-size 32 \
    ...
    --wandb-run-name critic_moral_stories_reborn \
    --save-results /scratch/mete/op/critic_moral_stories_reborn_outputs